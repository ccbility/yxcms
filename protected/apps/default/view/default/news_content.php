  ...
  <div class="line-middle">
    <div class="xm9">
      ...
      <div class="border padding">
           ...
           <p class="text-gray border-bottom border-dotted padding">发布日期：{date($info['addtime'],Y-m-d H:m:i)}&nbsp;&nbsp;点击量：{$info['hits']}&nbsp;&nbsp; 信息来源：{$info['origin']} </p>
         ...
         <ul class="list-unstyle border-top border-dotted padding text-gray">
              <li class="padding-little">上一篇：{if !empty($upnews)}<a href="{$upnews['url']}" onFocus="this.blur()">{$upnews['title']}</a>{else}没有了....{/if}</li>
              <li class="padding-little">下一篇：{if !empty($downnews)}<a href="{$downnews['url']}" onFocus="this.blur()">{$downnews['title']}</a>{else}没有了....{/if}</li>
          </ul>
