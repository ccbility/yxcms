...
class adminorderController extends appadminController
{
    public function index()
    {
        ...
        if (!empty($_GET['stype']) && !empty($_GET['keyword'])) {
            ...
            $url = url('adminorder/index',
                array('page' => '{page}', 'stype' => $stype, 'keyword' => urlencode($keyword)));
        }
    ...
    public function del()
    {
        if (!$this->isPost()) {
            ...
            if (!model('orderDetail')->delete("ordernum='{$info['ordernum']}'")) {
                echo '订单详细删除失败~';
                exit();
            }
            if (!model('orders')->delete("id='{$id}'")) {
                echo '订单删除失败~';
                exit();
            }
            echo 1;
            ...
            if (!model('orders')->delete("id in (" . $delid . ") AND state!='1' AND state!='2'")) {
                $this->error('删除订单失败~');
            }
            ...
            if (model('orderDetail')->delete("ordernum in (" . $orderstr . ")")) {
                $this->success('订单删除成功~');
            }
            $this->error('订单详细删除失败~');
        }
