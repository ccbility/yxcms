...
function urlRoute()
{
    ...
    if (!empty($rewrite)) {
        ...
        if (empty($_REQUEST['r']) && trim($_SERVER["REQUEST_URI"], '/')) {
            $_REQUEST['nor'] = true;
        }
    }
...
function url($route = 'index/index', $params = array())
{
    ...
    //如果开启了网址重写，伪静态
    if (!empty($rewrite)) {
        if (($app == 'default' && $route == 'index/index') || $route == 'default/index/index') {
            return 'http://' . $_SERVER["HTTP_HOST"];
        }
        static $urlArray = array();
        if (!isset($urlArray[$url])) {
            foreach ($rewrite as $rule => $mapper) {
                $mapper = '/' . str_ireplace(array('/', '<app>', '<c>', '<a>'),
                        array('\/', '(?<app>\w+)', '(?<c>\w+)', '(?<a>\w+)'), $mapper) . '/i';
                if (preg_match($mapper, $route, $matches)) {
                    list($app, $controller, $action) = explode('/', $route);
                    $urlArray[$url] = str_ireplace(array('<app>', '<c>', '<a>'), array($app, $controller, $action),
                        $rule);
                    if (!empty($params)) {
                        $_args = array();
                        ...
                        //处理多出来的参数
                        if (!empty($_args)) {
                            $urlArray[$url] = preg_replace('/<\w+>/', '',
                                    $urlArray[$url]) . '?' . http_build_query($_args);
                        }
                    }
                    //自动加上域名
                    if (false === stripos($urlArray[$url], 'http://')) {
                        $urlArray[$url] = 'http://' . $_SERVER['HTTP_HOST'] . rtrim(dirname($_SERVER["SCRIPT_NAME"]),
                                "./\\") . '/' . ltrim($urlArray[$url], "./\\");
                    }
...
function api($app, $method = '', $params = array())
{
    ...
    foreach ($apis as $app) {
        ...
        if (!is_object($objArray[$className])) {
            ...
            if (empty($config['APP_STATE']) || (1 != $config['APP_STATE'])) {
                continue;
            }
        ...
        if (!method_exists($objArray[$className], $method)) {
            continue;
        }
        config($config);
...
function run()
{
    ...
    try {
        ...
        if (is_mobile() && config(TPL_TEMPLATE_PATH_MOBILE)) {
            $tems = config(TPL_TEMPLATE_PATH_MOBILE);
        } else {
